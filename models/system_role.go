...
var t_system_role = "system_role"

func (r *SystemRole) GetRow() bool {
	has, err := mysqlClt.Get(r)
	if err == nil && has {
		return true
	}
	return false
...
func (r *SystemRole) GetRowByName() bool {
	role := SystemRole{}
	has, err := mysqlClt.Where("name = ?", r.Name).Get(&role)
	if err == nil && has {
		return true
	}
	return false
...
func (r *SystemRole) Add() bool {
	...
	role := SystemRole{}
	has, err := mysqlClt.Where("name = ?", r.Name).Desc("id").Get(&role)
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func (r *SystemRole) Delete() error {
	...
	//	return err
	//}
	if _, err := session.Exec("update "+t_system_role+" set status=? where id=?", 0, r.Id); err != nil {
		return err
	}
...
func (r *SystemRole) GetNameList() []string {
	var list []string
	err := mysqlClt.Table(t_system_role).Where("status=?", 1).Cols("name").Find(&list)
	if err != nil {
		panic(err)
	}
	return list
