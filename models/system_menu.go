...
var t_system_menu = "system_menu"

func (m *SystemMenu) GetRow() bool {
	has, err := mysqlClt.Get(m)
	if err == nil && has {
		return true
	}
	return false
...
func (m *SystemMenu) GetRowByPathCT(menu SystemMenu) bool {
	has, err := mysqlClt.Where("path=?", menu.Path).Where("component=?", menu.Component).Where("type=?", menu.Type).Get(m)
	if err == nil && has {
		return true
	}
	return false
...
func (m *SystemMenu) Add() (int64, error) {
	return mysqlClt.Insert(m)
}
func (m *SystemMenu) AddBatch(beans ...interface{}) (int64, error) {
	return mysqlClt.Insert(beans...)
}
func (m *SystemMenu) GetRowByUid(uid interface{}) ([]SystemMenu, error) {
	var menu []SystemMenu
	err := mysqlClt.Table(t_system_menu).Distinct(t_system_menu+".*").
		Join("INNER", systemrolemenu, systemrolemenu+".system_menu_id= "+t_system_menu+".id").
		Join("INNER", t_system_user_role, systemrolemenu+".system_role_id= "+t_system_user_role+".system_role_id").
		Where(t_system_menu+".status = ?", 1).
		Where(t_system_user_role+".system_user_id = ?", uid).
		Find(&menu)
	return menu, err
}
func (m *SystemMenu) GetRouteByUid(uid interface{}) (map[string]int, error) {
	...
	err := mysqlClt.Table(t_system_menu).Distinct(t_system_menu+".url", t_system_menu+".id").
		Join("INNER", systemrolemenu, systemrolemenu+".system_menu_id= "+t_system_menu+".id").
		Join("INNER", t_system_user_role, systemrolemenu+".system_role_id= "+t_system_user_role+".system_role_id").
		Where(t_system_menu+".status = ?", 1).
		Where(t_system_user_role+".system_user_id = ?", uid).
...
func (m *SystemMenu) GetRowByRole(id interface{}) ([]SystemMenu, error) {
	var menu []SystemMenu
	err := mysqlClt.Table(t_system_menu).Select(t_system_menu+".*").
		Join("INNER", systemrolemenu, systemrolemenu+".system_menu_id= "+t_system_menu+".id").
		Where(t_system_menu+".status = ?", 1).
...
func (rm *SystemMenu) GetAll() ([]SystemMenu, error) {
	...
	err := mysqlClt.Find(&systemmenus)
	return systemmenus, err
}

func (rm *SystemMenu) GetRowByType() ([]SystemMenu, error) {
	var systemmenus []SystemMenu
	err := mysqlClt.Where("type=?", rm.Type).Find(&systemmenus)
	return systemmenus, err
}

func (m *SystemMenu) Update() error {
	if _, err := mysqlClt.Where("id = ?", m.Id).Update(m); err != nil {
		return err
	}
	return nil
}
func (m *SystemMenu) Delete() error {
	if _, err := mysqlClt.Exec("update "+t_system_menu+" set status=? where id=?", 0, m.Id); err != nil {
		return err
	}
	return nil
}
