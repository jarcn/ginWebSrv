...
func (rm *SystemRoleMenu) Add() bool {
	...
	has, err := mysqlClt.Where("system_role_id = ?", rm.SystemRoleId).Where("system_menu_id=?", rm.SystemMenuId).Get(&rolemenu)
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func (rm *SystemRoleMenu) GetAll() ([]SystemRoleMenu, error) {
	...
	err := mysqlClt.Find(&systemrolemenus)
